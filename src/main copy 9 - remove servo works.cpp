...
void incomingStringProcessing(String receivingString ){
  ...
  if ( receivingString == "set2LOCK2set" ) {
    deviceLocked = true;
    servoWorksClose();
    digitalWrite (ledPin, HIGH);	// turn on the LED

  } else if ( receivingString ==  "set2UNLOCK2set" ) {
    deviceLocked = false;
    servoWorksOpen();
    digitalWrite (ledPin, LOW);	// turn off the LED
  } else if ( receivingString ==  "updateSTATUSupdate" ) {

    Serial.println("updateSTATUSupdate: ");

    if ( deviceLocked ){
      txValue = "LStatus__LOCKED__LStatus";
    } else {
      txValue = "LStatus__UNLOCKED__LStatus";
    }
  }
