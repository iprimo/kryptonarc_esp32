...
//     int ret = mbedtls_pk_parse_public_key(&pk, (const unsigned char*)PUBLIC_KEY, strlen(PUBLIC_KEY) + 1);
//     if (ret != 0) {
//         printf("mbedtls_pk_parse_public_key failed: %d\n", ret);
//         mbedtls_pk_free(&pk);
//         mbedtls_ctr_drbg_free(&ctr_drbg);
//         mbedtls_entropy_free(&entropy);
//         return -1;
...
//         return -4;
//     }
//     size_t key_size = mbedtls_pk_get_len(&pk);
//     if (key_size > 256) key_size = 256; // limit for 2048-bit
//     unsigned char encrypted[256];
//     size_t olen = 0;
//     ret = mbedtls_pk_encrypt(&pk, (const unsigned char*)input, strlen(input), encrypted, &olen, sizeof(encrypted), mbedtls_ctr_drbg_random, &ctr_drbg);
//     if (ret != 0) {
//         printf("mbedtls_pk_encrypt failed: %d\n", ret);
//         mbedtls_pk_free(&pk);
//         mbedtls_ctr_drbg_free(&ctr_drbg);
//         mbedtls_entropy_free(&entropy);
//         return -2;
